...
import { ELanguage, keysLanguage } from '../Languages';
import { getMenu, TMenuState } from '../Menus';
...
export type TKeysKey = { className: string; code: string; character: string };
export type TKeysKeys = { [key: string]: TKeysKey[] };
export type TKeysLanguage = { [key in ELanguage]: TKeysKeys };
...
const keyType = (
  ...
  if (!isNaN(dataCharacterCode)) {
    ...
    if (menu.menuState.diacriticKey > 0) {
      message.messageState.push({
        menuKey: menu.menuState.diacriticKey,
        diacriticKey: dataCharacterCode,
      });
      message.setMessageState([...message.messageState]);
      menu.setMenuState({ menuKey: 1000, diacriticKey: 0 });
    }
...
export const buildKeys = (
  ...
  const keysCharacters: TKeysKeys = keysLanguage[language];
  const keys: React.ReactNode[] = [];
  const rows: string[] = ['row1', 'row2', 'row3', 'row4', 'row5'];

  rows.forEach((row) => {
    const buildKey = (key: TKeysKey, index: number): JSX.Element => {
      ...
      if (row === 'row5') {
        if (['ControlLeft', 'AltLeft'].indexOf(key.code) > -1)
          finalClass = 'key--w1 key--placeholder';
        if (key.code === 'Space') finalClass = key.className;
        if (['ControlRight', 'AltRight', 'X1', 'X2', 'X3'].indexOf(key.code) > -1)
          finalClass = 'key--placeholder';
        ...
        >
          ...
          <svg fill={'#eee'} width={'2.5em'} height={'2.5em'}>
            {menuCharacter && menu.menuState.diacriticKey === 0 && (
              <use href={`#${menuCharacter.toString()}`}></use>
            )}
    ...
    keys.push(
      <div key={row} className='keys__row'>
        {keysCharacters[row].map(buildKey)}
      </div>,
    );
  });
